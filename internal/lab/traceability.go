...
func (i *TraceabilityIndex) Snapshot(profileID string) TraceabilityView {
	i.mu.RLock()
	defer i.mu.RUnlock()
	source := i.ByProfile[profileID]
	records := make([]TraceabilityRecord, len(source))
	copy(records, source)
	return TraceabilityView{Records: records, Revision: i.Revision}
}

func (i *TraceabilityIndex) Count() int {
	i.mu.RLock()
	defer i.mu.RUnlock()
